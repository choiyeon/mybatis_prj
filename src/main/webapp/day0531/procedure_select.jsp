...
<input type="hidden" name="link" value="day0531/procedure_select"/><!-- include할 파라메터 설정 -->
<h2>사원정보 조회 </h2>
<label>부서번호</label>
...
<div>
	<c:if test="${ not empty param.deptno }">
			<strong><c:out value="${ param.deptno }" /></strong>부서의 검색결과 
		<c:catch var="errmsg"><!-- java의 try/catch 비슷한 거. 에러가 발생할 수 있는 부분 -->
			<%
			ExamMapper7DAO emp7DAO = ExamMapper7DAO.getInstance();
			List<EmpDomain> list = emp7DAO.selectProcedure(Integer.parseInt(deptno));
